...
-- 0) Secuencia y trigger para número (si no estaba)
...
do $$ begin
  if exists (select 1 from information_schema.tables where table_name='supplier_types') then
    alter table public.clients
      add constraint if not exists clients_supplier_type_fk
      foreign key (supplier_type_id) references public.supplier_types(id) on delete set null;
  end if;
end $$;
